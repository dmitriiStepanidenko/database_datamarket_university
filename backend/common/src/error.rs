...
//impl<T> FromIterator<Result<T>> for Result<T> {}

// for slightly less verbose error mappings
pub trait IntoApiError {
    fn into_api_error(self, ctx: &dyn Ctx) -> ApiError;
}
...
impl ApiError {
    pub fn from<T: Into<Error>>(ctx: &dyn Ctx) -> impl FnOnce(T) -> ApiError + '_ {
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Forbidden => write!(f, "Forbidden! You do not have needed priveledges"),
        }
    }
}
